...
var SYSTEM = {
	...
	simInit: func() {
		...
		if (!CONFIG.noUpdateCheck) {
			http.load("https://raw.githubusercontent.com/Octal450/DC-10/master/revision.txt").done(func(r) me.newRevision.setValue(r.response));
		}
...
var OPTIONS = {
	...
	read: func() {
		io.read_properties(getprop("/sim/fg-home") ~ "/Export/DC-10-options.xml", "/systems/acconfig/temp");
		...
		if (me.tempRevision.getValue() < CONFIG.minOptionsRevision) {
			...
			io.read_properties(getprop("/sim/fg-home") ~ "/Export/DC-10-options.xml", "/systems/acconfig/options");
...
var PANEL = {
	...
	panelBase: func(t, s = 0) {
		...
		if (t == 1) {
			pts.Controls.Flight.elevatorTrim.setValue(math.round(pts.Fdm.JSBSim.Performance.stabilizerDeg.getValue(), 0.1) / -15); # Trim value for CG
			pts.Controls.Flight.flaps.setValue(0.36); # 10-25/EXT
			pts.Controls.Flight.speedbrakeArm.setBoolValue(1);
		} else {
			pts.Controls.Flight.elevatorTrim.setValue(-0.2); # About 3ANU
